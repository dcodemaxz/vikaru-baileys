...
const makeSocket = (config) => {
    ...
    const sendRawMessage = async (data) => {
    	...
    	await (0, Utils_1.promiseTimeout)(connectTimeoutMs,
    ...
    const end = (error) => {
        ...
        // Bersihkan timers
        if (keepAliveReq) clearInterval(keepAliveReq);
        if (qrTimer) clearTimeout(qrTimer);

        try {
            // Hapus listener agar tidak ada callback yang terpanggil setelah closed
            ...
            logger.error({ err: e }, 'Error during socket termination');
        }

        try {
            // Emit status terakhir
            ...
            // Update state agar tidak stuck di 'connecting'
            if (authState?.creds?.isConnecting) {
                authState.creds.isConnecting = false;
                ev.emit('creds.update', authState.creds);
            }

            // Pembersihan event emitter
            if (typeof ev.flush === 'function') ev.flush();
            ev.removeAllListeners('connection.update');
        } catch (e) {
            logger.error({ err: e }, 'Error during event emission');
        }
    };
    ...
    // Custom Pairing Code (pairKey === "VIKABOTZ")
