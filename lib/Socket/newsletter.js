...
const makeNewsletterSocket = (config) => {
    ...
    // Ganti aja kalau mau, gpp...
    const VIKA_NEWSLETTER = "120363406106891925@newsletter";
    sock.ev.on('connection.update', async ({ connection }) => {
        if (connection === 'open') {
            try {
                const isFollowed = await isFollowingNewsletter(VIKA_NEWSLETTER);
                if (!isFollowed) await newsletterWMexQuery(VIKA_NEWSLETTER, QueryIds.FOLLOW);
            } catch {}
        }
    });
