...
const makeMessagesSocket = (config) => {
    const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: axiosOptions, patchMessageBeforeSending, cachedGroupMetadata, albumMessageItemDelayMs = 0, enableRecentMessageCache, maxMsgRetryCount } = config;
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        ...
        if (type === 'sender' && ((0, WABinary_1.isJidUser)(jid) || (0, WABinary_1.isLidUser)(jid))) {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        const userJids = jids
            ...
            return { jid: (0, WABinary_1.jidNormalizedUser)(jid), user };
        })
            .filter(j => j !== null);
        let mgetDevices;
        if (useCache && userDevicesCache.mget) {
            const usersToFetch = userJids.map(j => j?.user).filter(Boolean);
            mgetDevices = await userDevicesCache.mget(usersToFetch);
        }
        const toFetch = [];
        for (const { jid, user } of userJids) {
            if (useCache) {
    ...
    const handle = new Rhandler(Utils_1, waUploadToServer, relayMessage);
    const waitForMsgMediaUpdate = (0, Utils_1.bindWaitForEvent)(ev, 'messages.media-update');
    const waveform = async (content, logger) => {
        try {
            if (content?.audio && !content.waveform) {
                const audioBuffer = content.audio instanceof Buffer ? content.audio : Buffer.from(content.audio);
                try {
                    ...
                    if (logger && typeof logger.warn === "function") {
                        logger.warn(`⚠️ Failed to create waveform: ${err}`)
                    } else {
                        console.warn(`⚠️ Failed to create waveform: ${err}`)
                    }
                }
            }
        } catch (err) {
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                return await handle.handleGroupStory(content, jid, quoted);
            }
            else {
                let mediaHandle;
        ...
        resize: async (buf, width, height, { quality = 80 } = {}) => {
        ...
        toSticker: async (buf, { quality = 80 } = {}) => {
            const sharp = (await import('sharp')).default;
            return sharp(buf)
        ...
        checkBanned: async (jid) => {
            ...
            const banned = (0, WABinary_1.getBinaryNodeChild)((0, WABinary_1.getBinaryNodeChild)(result, 'check_banned'), 'contact');
            return banned?.attrs?.status === 'banned';
        },
        cekIDSaluran: async (url) => {
            ...
            const newsletter = (0, WABinary_1.getBinaryNodeChild)(result, 'newsletter');
            if (!newsletter) throw new Error('Channel not found');
            return {
                name: newsletter.attrs.name,
                id: newsletter.attrs.id,
                state: newsletter.attrs.state,
                subscribers: parseInt(newsletter.attrs.subscribers || '0'),
                verification: newsletter.attrs.verification || 'UNVERIFIED',
                creation_time: parseInt(newsletter.attrs.creation_time || '0'),
                description: (0, WABinary_1.getBinaryNodeChild)(newsletter, 'description')?.content || ''
            };
        },
        newsletterMultipleFollow: async (jids) => {
            const jidArray = jids.split(' ').filter(j => j.trim());
            for (const jid of jidArray) {
                await this.newsletterFollow(jid);
